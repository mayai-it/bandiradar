...
| **`anac_pvl`** | ANAC *Pubblicità a Valore Legale* — the national feed of **OPEN Italian public tenders** (`kind="tender"`), incl. **sub-threshold** ones TED never lists; notices stay online until their deadline. This is the live open-calls feed the others lack. Carries buyer, oggetto, CIG, importo (sparse), province→region. *Caveats:* CPV is the Italian **label** only (numeric code = next slice); importo often absent; region via a static province table. | ✅ Wired — public JSON API, **no credentials**; keeps only still-open gare (deadline in the future). |
